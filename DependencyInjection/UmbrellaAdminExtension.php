...
class UmbrellaAdminExtension extends Extension implements PrependExtensionInterface
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        $parameters = ArrayUtils::remap_nested_array($config, 'umbrella_admin', ['umbrella_admin.user.group.form_roles']);
        ...
        if ($config['user']['group']['enabled']) {
            $this->enableUserGroup($container, $config['user']['group']);
        }
    ...
    public function prepend(ContainerBuilder $container)
    {
        ...
        //             Umbrella\AdminBundle\Model\AdminUserInterface : <value of umbrella_admin.user.user.class config>
        //
        $configs = $container->getExtensionConfig('umbrella_admin');
        $config = $this->processConfiguration(new Configuration(), $configs);

        $doctrineConfig = [];
        $doctrineConfig['orm']['resolve_target_entities'][AdminUserInterface::class] = $config['user']['user']['class'];
